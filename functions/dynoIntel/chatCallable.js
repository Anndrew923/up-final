...
async function consumeDynoQuota(uid, isPro, requestId, now) {
  return db.runTransaction(async (tx) => {
    const { ref, data } = await loadDynoRateLimitDoc(uid, tx);
    const rateCheck = reserveDynoIntelUsage(data, isPro, requestId, now);
    if (!rateCheck.allowed) {
      if (rateCheck.reason === "in-progress") {
    ...
    tx.set(
      ref,
      {
        dayKey: rateCheck.bucket.dayKey,
        countToday: rateCheck.bucket.countToday,
        limit: rateCheck.bucket.limit,
        pendingReservations: rateCheck.bucket.pendingReservations,
      },
      { merge: true }
    );
    return {
      ...
      limit: rateCheck.limit,
      quotaTier: rateCheck.quotaTier,
      dayKey: rateCheck.dayKey,
      resetAt: rateCheck.resetAt,
    };
  });
}

async function completeDynoChatSuccess(uid, isPro, requestId, reply, options) {
  const quota = await finalizeDynoQuotaReservation(uid, isPro, requestId);
  return buildDynoChatSuccess(quota, reply, options);
}

async function releaseQuotaAfterFailure(uid, isPro, requestId, error) {
  try {
    await releasePendingDynoQuotaReservation(uid, isPro, requestId);
  } catch (releaseError) {
    console.error("[dynoIntelChat] quota release failed", releaseError?.message ?? releaseError);
  }
  throw error;
}
...
export const dynoIntelChat = onCall(
  ...
  async (request) => {
    ...
    const requestId =
      typeof data.requestId === "string" && /^[A-Za-z0-9_-]{8,64}$/.test(data.requestId)
        ? data.requestId
        : randomUUID();
    ...
    const priorFocusAxis =
      priorTurnRaw && typeof priorTurnRaw === "object" && typeof priorTurnRaw.focusAxis === "string"
    ...
    const now = new Date();
    const { isPro, hasCore, hasProQuota } = await resolveDynoIntelEntitlement(uid, now);
    ...
    if (cached) {
      ...
      try {
        ...
        return releaseQuotaAfterFailure(uid, hasProQuota, requestId, error);
      }
    }

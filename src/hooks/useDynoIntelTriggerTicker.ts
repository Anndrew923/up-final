...
export interface UseDynoIntelTriggerTickerOptions {
  ...
  enabled?: boolean;
  /** Freeze on verb CTA and skip rotation while first-run discovery is active. */
...
export function useDynoIntelTriggerTicker(
  ...
  const { enabled = true, discoveryMode = false } = options;
  const { t } = useTranslation('common');
  const reducedMotion = usePrefersReducedMotion();
  const [index, setIndex] = useState(0);

  useEffect(() => {
    if (reducedMotion || !enabled || discoveryMode) return;
    const id = window.setInterval(
      () => setIndex((current) => (current + 1) % TICKER_ENTRIES.length),
      DYNO_INTEL_TRIGGER_TICKER_INTERVAL_MS,
    );
    return () => window.clearInterval(id);
  }, [discoveryMode, enabled, reducedMotion]);

  if (discoveryMode) {
    return {
      label: t('dynoIntel.triggerDiscovery.cta'),
      phase: 'scanning',
      index: 0,
      reducedMotion,
    };

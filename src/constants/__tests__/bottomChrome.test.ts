...
describe('bottomChrome', () => {
  ...
  it('clears Dyno Intel chip for default AppShell scroll inset', () => {
    expect(APP_SHELL_SCROLL_BOTTOM_PX).toBe(BOTTOM_CHROME_STACK_PX + DYNO_INTEL_TRIGGER_HEIGHT_PX);
  });
  ...
  it('parks filter pill above scroll chrome (bridge or Dyno stack)', () => {
    expect(LADDER_FILTER_PILL_BOTTOM_PX).toBe(
      LADDER_SCROLL_BOTTOM_INSET_PX + LADDER_FILTER_PILL_AIR_GAP_PX
    );
  });
